...
from .ring import NTT, find_ntt_primes
...
class RnsRing:
    ...
    def mul_hat(self, ah, bh):
        return (ah * bh) % self.qs
    ...
    def inf_norm(self, a):
        c = self.center(a)
        return max(abs(int(v)) for v in c) if len(c) else 0
    ...
    def is_invertible_hat(self, ah):
        return bool(np.all(ah != 0))
...
def default_ring(n=4096, nprimes=4, bits=20):
    return RnsRing(n, find_ntt_primes(n, bits, nprimes))
...
def center_stack(R, xs):
    return np.stack([np.array([int(v) for v in R.center(xs[j])], dtype=object)
